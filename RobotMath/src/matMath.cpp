...
#pragma region General Matrix
	...
	double Matrix::determinant(int n) {
		...
		else
		{
			...
			if (n == 2)
			{
				det = this->matrixA[0][0] * this->matrixA[1][1] - this->matrixA[1][0] * this->matrixA[0][1];
				return det;
			}
			else
			{
				for (int x = 0; x < n; x++) {
					...
					det = det + (pow(-1, x) * this->matrixA[0][x] * submatrix.determinant(n - 1));
				}
			}
			return det;
		}
		...
		if (matA.cols == matB.rows)
		{
			...
			for (unsigned int r = 0; r < res.rows; r++)
			{
				for (unsigned int c = 0; c < res.cols; c++)
				{
					...
					{
						res.matrixA[r][c] += matA.matrixA[r][i] * matB.matrixA[i][c];
					}
				}
			}
			return res;
...
#pragma region DCM
	DCM::DCM(double* n, double theta, double* trans_vec) : Matrix(4, 4)
	{
